...
module.exports = async function (context, req) {
    ...
    try {
        ...
        for (const item of feed.items) {
            // 1. Clean the link (Removes ?source=rss which triggers the snippet-only wall)
            const cleanLink = item.link.split('?')[0];
            context.log(`Scraping Full Text for: ${item.title}`);

            let fullArticleText = "";
            try {
                // Wait 1 second between requests to avoid 429 Rate Limits
                ...
                // 2. Targeted Selector: Medium wraps main story text in 'p' tags within 'article' or 'section'
                let paragraphs = [];
                ...
                // Fallback if scraping is blocked or empty
                if (!fullArticleText || fullArticleText.length < 500) {
                    fullArticleText = item.content || item.contentSnippet;
                }
            } catch (e) {
                context.log.error(`Scrape failed for ${item.title}: ${e.message}`);
            ...
            posts.push({ title: item.title, status: "Synced" });
        }
